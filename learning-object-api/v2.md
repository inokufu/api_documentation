...
Our mission is to provide you with the best learning objects out there. While we are working hard on this ambitious goal, we are still at the begining of this journey and we  don't have indexed the whole educational content available on internet \(yet\). , we can't guaranty the accuracy of our results, nor that we will be able to find learning objects on any topic or set of query parameters.

Each response returned by the endpoint `search` and `search-provider` contains a message \(`msg`\) that helps you understand how good we think our answer to your query is. Messages currently returned by the API are shown in the table below.
...
| B00 | These results does not match exactly your query. | We have some results that may be close enough to your query but still it's not exactly what you asked for, so you know. |
...
| Parameter | Required | Description |
| :--- | :--- | :--- |
| `url` | Yes | Url of the learning object you want to give feedback on. |
| `comment` | No | Enter any comment associated to the learning object. |
| `request_id` | No | Enter the request id of the initial request that sent you this learning object in the response. It will help us identify any issue related with the request. |
